...
# class ShowResultView(generic.DetailView):
#     model = Submission
...
def extract_answers(request):
   ...
   for key, value in request.POST.items():
       if key.startswith('choice'):
           ...
           submitted_anwsers[choice_id] = True
...
def submit(request, course_id):
    ...
    return redirect('onlinecourse:show_result', course_id=course.id, submission_id=submission.id)
...
def show_exam_result(request, course_id, submission_id):
    ...
    user = request.user
    # Retrieve the selected choice IDs from the submission record
    selected_choice_ids = submission.chocies.values_list('id', flat=True)
    total_score = 0
    total_missed = 0
    ...
    grade = ( total_score / (total_score + total_missed) ) * 100
    selected_ids = list(selected_choice_ids)
    context = {
        'user': user,
        'course': course,
        'questions': questions,
